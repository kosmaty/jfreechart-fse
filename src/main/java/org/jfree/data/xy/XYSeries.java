...
public class XYSeries extends Series implements Cloneable, Serializable {
	...
	/**
	 * Returns a flag that controls whether duplicate x-values are allowed. The
	 * default value is {@code true}. This flag can only be set in the
	 * constructor. When it is set to {@code false}, an exception will be thrown
	 * if you attempt to add an item with an x-value that already exists in the
	 * series.
	 *
	 * @return A boolean.
	 */
	 ...
	 *             <code>allowDuplicateXValues</code> flag is not set for this
	 *             series.
	 */
	 ...
	 * {@link SeriesChangeEvent} to all registered listeners. Note that when a
	 * series permits multiple items with the same x-value, this method could
	 * remove any one of the items with that x-value.
	 *
	 * @param x
	 *            the x-value.
	 * 
