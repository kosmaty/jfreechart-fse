...
public interface WithChangeListener<T extends EventListener> {
	 ...
	 * <br>
	 * The listener is triggered if if changes occur except the notify flag is
	 * set to false (@link #setNotify(boolean)). In the latter case a change
	 * event should be triggered as soon as the notify flag is set to true
	 * again.
	 * 
	 * @param listener
	 *            the listener.
	 ...
	 *
	 * @param notify
	 *            If the flag is set to false the listeners are no longer
	 *            informed about changes. If the flag is set to true and some
	 *            changes occurred an event should be triggered.
	 */
