...
/**
 * A simple implementation of the {@link BoxAndWhiskerXYDataset} interface. This
 * dataset implementation can hold only one series.
 */
public class DefaultBoxAndWhiskerXYDataset extends AbstractXYDataset
	...
	/**
	 * The coefficient used to calculate outliers. Tukey's default value is 1.5
	 * (see EDA) Any value which is greater than Q3 + (interquartile range *
	 * outlier coefficient) is considered to be an outlier. Can be altered if
	 * the data is particularly skewed.
	 */
	 ...
	 * coefficient gives an indication of the degree of certainty in an unskewed
	 * distribution. Increasing the coefficient increases the number of values
	 * included. Currently only used to ensure farout coefficient is greater
	 * than the outlier coefficient
	 *
	...
	/**
	 * Adds an item to the dataset and sends a {@link DatasetChangeEvent} to all
	 * registered listeners.
	 *
	 * @param date
	 *            the date (<code>null</code> not permitted).
	 * @param item
	 *            the item (<code>null</code> not permitted).
	 */
	 ...
	 *
	 * @param series
	 *            the series index (ignored, since this dataset contains only
	 *            one series).
	 * @param item
	 ...
	 *            the item (zero-based index).
	 *
	 * @return The list of outliers for the specified series and item (possibly
	 *         <code>null</code>).
	 */
	@Override
