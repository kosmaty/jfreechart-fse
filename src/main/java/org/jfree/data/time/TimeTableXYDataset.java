 ...
 * interface. Note that the {@link TableXYDataset} interface requires all series
 * to share the same set of x-values. When adding a new item {@code (x, y)} to
 * one series, all other series automatically get a new item {@code (x, null)}
 * unless a non-null item has already been specified.
 *
...
public class TimeTableXYDataset extends AbstractIntervalXYDataset
	 ...
	 * (elsewhere in JFreeChart rows are typically used for series, but it
	 * doesn't matter that much since this data structure is private and
	 * symmetrical anyway), each row contains values for the same
	 ...
	 * collection is used as an {@link org.jfree.data.xy.XYDataset}. This can be
	 * the start, middle or end of the time period.
	 */
	 ...
	 *
	 * @param zone
	 *            the time zone to use ({@code null} not permitted).
	 */
	 ...
	 *            the time zone to use ({@code null} not permitted).
	 * @param locale
	 *            the locale to use ({@code null} not permitted).
	 */
	 ...
	 * @param y
	 *            the value for this period ({@code null} permitted).
	 * @param seriesName
	 *            the name of the series to add the value ({@code null} not
	 *            permitted).
	 * @param notify
	 ...
	 * @param seriesName
	 *            the (existing!) series name to remove the value ({@code null}
	 *            not permitted).
	 *
	...
	/**
	 * Removes an existing data item from the dataset and, if requested, sends a
	 * {@link DatasetChangeEvent} to all registered listeners.
	 *
	 * @param period
	 *            the (existing!) time period of the value to remove (
	 ...
	 * returned by {@link #getItemCount()} since all series share the same
	 ...
	 *
	 * @param includeInterval
	 *            a flag that controls whether or not the x-intervals are taken
	 *            into account.
	 *
	 * @return The range.
	 */
