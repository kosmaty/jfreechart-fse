...
public class CategoryToPieDataset extends AbstractDataset
	 ...
	 * <p>
	 * If <code>source</code> is <code>null</code>, the created dataset will be
	 * empty.
	 *
	 * @param source
	...
	/**
	 * Returns the extract type, which determines whether data is read from one
	 * row or one column of the underlying dataset.
	 *
	 ...
	 *             if <code>item</code> is not in the range <code>0</code> to
	...
	/**
	 * Returns the index for a given key, or <code>-1</code> if there is no such
	 * key.
	 *
	 * @param key
	 *            the key.
	...
	/**
	 * Sends a {@link DatasetChangeEvent} to all registered listeners, with this
	 * (not the underlying) dataset as the source.
	 *
	 * @param event
