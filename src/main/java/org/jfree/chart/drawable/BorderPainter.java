...
public class BorderPainter implements Drawable, Serializable {
	...
	public BorderPainter() {
		this(Color.BLACK, new StrokeProperties(1.0));
	}
	 ...
	 */
	public BorderPainter(Color color, StrokeProperties stroke) {
	 ...
	 *            the stroke (<code>null</code> not permitted).
	 * @param arcH
	 *            the arc height (for rounded corners).
	 * @param arcW
	 *            the arc width (for rounded corners).
	 */
	public BorderPainter(Color color, StrokeProperties stroke, double arcH, double arcW) {
		this.color = color;
		this.stroke = stroke;
		this.arcHeight = arcH;
		this.arcWidth = arcW;
	...
	/**
	 * Returns the arc height for rounded corners. If this is 0.0, the border
	 * will have square corners. The default value is 0.0.
	 * 
	...
	/**
	 * Returns the arc width for rounded corners. If this is 0.0, the border
	 * will have square corners. The default value is 0.0.
	 * 
	 * @return The arc height.
	 */
	 ...
	 * @param area
	 *            the area for the border (<code>null</code> not permitted).
	 */
	@Override
	public void draw(GraphicsContext g2, Rectangle2D area) {
		RectangularShape border;
		if (this.arcHeight > 0.0 && this.arcWidth > 0.0) {
			border = new RoundRectangle2D(0f, 0f, 0f, 0f,
					(float) this.arcWidth, (float) this.arcHeight);
		} else {
			border = new RoundRectangle2D(0f, 0f, 0f, 0f, 0f, 0f);
		}
		border.setFrame((float) area.getMinX(), (float) area.getMinY(), (float) (area.getWidth() - 1.0),
				(float) (area.getHeight() - 1.0));
		g2.setStroke(this.color);
		setStrokeProperties(g2, this.stroke);
		outlineShape(g2, border);
	}
