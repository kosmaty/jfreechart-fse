 ...
 * code in a <a
 * href="http://www.jroller.com/gfx/entry/fast_or_good_drop_shadows">blog post
 * by Romain Guy</a>.
 *
 * @since 1.0.14
 */
public class DefaultShadowGenerator implements ShadowGenerator, Serializable {
	 ...
	 */
	@Override
	public Image createDropShadow(Image _source) {

		BufferedImage source = SwingFXUtils.fromFXImage(_source, null);
		BufferedImage subject = new BufferedImage(
				(int) source.getWidth() + this.shadowSize * 2,
				(int) source.getHeight() + this.shadowSize * 2,
		...
		g2.dispose();
		applyShadow(subject);
		return SwingFXUtils.toFXImage(subject, null);
	}
