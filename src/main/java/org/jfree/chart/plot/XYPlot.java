...
public class XYPlot extends Plot implements ValueAxisPlot, Pannable, Zoomable,
	...
	private transient Point2D quadrantOrigin = new Point2D(0.0, 0.0);
	 ...
	 */
	public int indexOf(XYDataset dataset) { // FIXME: rename this
											// findDatasetIndex()
	 ...
	 *
	 * @param renderer
	 *            the renderer ({@code null} permitted).
	 *
	 * @see #getRenderer()
	 */
	 ...
	 */
	public int getIndexOf(XYItemRenderer renderer) { // FIXME : rename
														// findRendererIndex()
	 ...
	 */
	protected AxisSpace calculateAxisSpace(GraphicsContext g2,
	 ...
	 * @return The required space.
	 */
	protected AxisSpace calculateDomainAxisSpace(GraphicsContext g2,
	 ...
	 * @return The required space.
	 */
	protected AxisSpace calculateRangeAxisSpace(GraphicsContext g2,
			Rectangle2D plotArea, AxisSpace space) {
	 ...
	 */
	private Rectangle2D integerise(Rectangle2D rect) {
	 ...
	 */
	@Override
	public void draw(GraphicsContext g2, Rectangle2D area, Point2D anchor,
		...
		createAndAddEntity(dataArea, info, null, null);
		if (info != null) {
	 ...
	 */
	@Override
	public void drawBackground(GraphicsContext g2, Rectangle2D area) {
	 ...
	 * @see #setQuadrantPaint(int, Paint)
	 */
	protected void drawQuadrants(GraphicsContext g2, Rectangle2D area) {
		// 0 | 1
		// --+--
		// 2 | 3
		...
		if (this.quadrantPaint[0] != null) {
			if (x > xmin && y < ymax) {
				if (this.orientation == PlotOrientation.HORIZONTAL) {
					r[0] = new Rectangle2D(Math.min(yymax, yy),
							Math.min(xxmin, xx), Math.abs(yy - yymax),
							Math.abs(xx - xxmin));
				}
				else { // PlotOrientation.VERTICAL
					r[0] = new Rectangle2D(Math.min(xxmin, xx),
		...
		if (this.quadrantPaint[1] != null) {
			if (x < xmax && y < ymax) {
				if (this.orientation == PlotOrientation.HORIZONTAL) {
					r[1] = new Rectangle2D(Math.min(yymax, yy),
							Math.min(xxmax, xx), Math.abs(yy - yymax),
							Math.abs(xx - xxmax));
				}
				else { // PlotOrientation.VERTICAL
					r[1] = new Rectangle2D(Math.min(xx, xxmax),
		...
		if (this.quadrantPaint[2] != null) {
			if (x > xmin && y > ymin) {
				if (this.orientation == PlotOrientation.HORIZONTAL) {
					r[2] = new Rectangle2D(Math.min(yymin, yy),
							Math.min(xxmin, xx), Math.abs(yy - yymin),
							Math.abs(xx - xxmin));
				}
				else { // PlotOrientation.VERTICAL
					r[2] = new Rectangle2D(Math.min(xxmin, xx),
		...
		if (this.quadrantPaint[3] != null) {
			if (x < xmax && y > ymin) {
				if (this.orientation == PlotOrientation.HORIZONTAL) {
					r[3] = new Rectangle2D(Math.min(yymin, yy),
							Math.min(xxmax, xx), Math.abs(yy - yymin),
							Math.abs(xx - xxmax));
				}
				else { // PlotOrientation.VERTICAL
					r[3] = new Rectangle2D(Math.min(xx, xxmax),
	 ...
	 * @see #setDomainTickBandPaint(Paint)
	 */
	public void drawDomainTickBands(GraphicsContext g2, Rectangle2D dataArea,
	 ...
	 * @see #setRangeTickBandPaint(Paint)
	 */
	public void drawRangeTickBands(GraphicsContext g2, Rectangle2D dataArea,
			List<ValueTick> ticks) {
	 ...
	 */
	protected Map<Axis, AxisState> drawAxes(GraphicsContext g2, Rectangle2D
			plotArea,
	 ...
	 */
	protected void drawRangeGridlines(GraphicsContext g2, Rectangle2D area,
			List<ValueTick> ticks) {
	 ...
	 * @since 1.0.5
	 */
	protected void drawZeroDomainBaseline(GraphicsContext g2, Rectangle2D area) {
	 ...
	 *             this can occur if some component of the plot cannot be
	 *             cloned.
	 */
	@Override
