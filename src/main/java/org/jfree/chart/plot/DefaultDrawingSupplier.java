...
public class DefaultDrawingSupplier implements DrawingSupplier, Cloneable,
	...
	public static Shape[] createStandardSeriesShapes() {
		...
		float size = 6.0f;
		float delta = size / 2.0f;
		float[] xpoints = null;
		float[] ypoints = null;

		// square
		result[0] = new RoundRectangle2D(-delta, -delta, size, size, 0f, 0f);
		// circle
		result[1] = new Ellipse2D(-delta, -delta, size, size);

		// up-pointing triangle
		xpoints = new float[] { 0.0f, delta, -delta };
		ypoints = new float[] { -delta, delta, delta };
		result[2] = ShapeUtils.newPolygon(xpoints, ypoints);

		// diamond
		xpoints = new float[] { 0.0f, delta, 0.0f, -delta };
		ypoints = new float[] { -delta, 0.0f, delta, 0.0f };
		result[3] = ShapeUtils.newPolygon(xpoints, ypoints);

		// horizontal rectangle
		result[4] = ShapeUtils.asShape(new Rectangle2D(-delta, -delta / 2, size, size / 2));

		// down-pointing triangle
		xpoints = new float[] { -delta, +delta, 0.0f };
		ypoints = new float[] { -delta, -delta, delta };
		result[5] = ShapeUtils.newPolygon(xpoints, ypoints);

		// horizontal ellipse
		result[6] = new Ellipse2D(-delta, -delta / 2, size, size / 2);

		// right-pointing triangle
		xpoints = new float[] { -delta, delta, -delta };
		ypoints = new float[] { -delta, 0.0f, delta };
		result[7] = ShapeUtils.newPolygon(xpoints, ypoints);

		// vertical rectangle
		result[8] = ShapeUtils.asShape(new Rectangle2D(-delta / 2, -delta, size / 2, size));

		// left-pointing triangle
		xpoints = new float[] { -delta, delta, delta };
		ypoints = new float[] { 0.0f, -delta, +delta };
		result[9] = ShapeUtils.newPolygon(xpoints, ypoints);
		return result;
	}
