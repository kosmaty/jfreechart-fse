...
public class BarRenderer extends AbstractCategoryItemRenderer
	...
	public BarRenderer() {
		...
		setDefaultLegendShape(asShape(new Rectangle2D(-4.0, -4.0, 8.0, 8.0)));
	 ...
	 *            include the interval if the dataset has one?
	 *
	...
	public LegendItem getLegendItem(int datasetIndex, int series) {
		...
		LegendItem result = new LegendItem(label, description, toolTipText,
				...
				outlinePaint, outlineStroke, false, new Line2D(),
				new StrokeProperties(1.0f), Color.BLACK);
	 ...
	 */
	@Override
	public void drawItem(GraphicsContext g2,
			CategoryItemRendererState state,
			Rectangle2D dataArea,
			CategoryPlot plot,
			CategoryAxis domainAxis,
		...
		Rectangle2D bar;
		if (orientation == PlotOrientation.HORIZONTAL) {
			bar = new Rectangle2D(barL0 - barL0Adj, barW0,
					barLength + barLengthAdj, state.getBarWidth());
		}
		else {
			bar = new Rectangle2D(barW0, barL0 - barL0Adj,
					state.getBarWidth(), barLength + barLengthAdj);
		}
		if (getShadowsVisible()) {
			this.barPainter.paintBarShadow(g2, this, row, column, asShape(bar), barBase,
					true);
		}
		this.barPainter.paintBar(g2, this, row, column, asShape(bar), barBase);
		...
		if (entities != null) {
			addItemEntity(entities, dataset, row, column, asShape(bar));
		}
	 ...
	 */
	protected void drawItemLabel(GraphicsContext g2,
		...
		if (isInternalAnchor(position.getItemLabelAnchor())) {
			...
			if (bounds != null) {
				if (!bar.contains(asRectangle2D(bounds.getBounds()))) {
					if (!negative) {
	...
	private Point2D calculateLabelAnchorPoint(ItemLabelAnchor anchor,
		...
		double x0 = bar.getMinX() - offset;
		double x1 = bar.getMinX();
		double x2 = bar.getMinX() + offset;
		double x3 = getCenterX(bar);
		...
		double y3 = getCenterY(bar);
		double y4 = bar.getMinY() + offset;
		double y5 = bar.getMinY();
		double y6 = bar.getMinY() - offset;

		if (anchor == ItemLabelAnchor.CENTER) {
			result = new Point2D(x3, y3);
		}
		else if (anchor == ItemLabelAnchor.INSIDE1) {
			result = new Point2D(x4, y4);
		}
		else if (anchor == ItemLabelAnchor.INSIDE2) {
			result = new Point2D(x4, y4);
		}
		else if (anchor == ItemLabelAnchor.INSIDE3) {
			result = new Point2D(x4, y3);
		}
		else if (anchor == ItemLabelAnchor.INSIDE4) {
			result = new Point2D(x4, y2);
		}
		else if (anchor == ItemLabelAnchor.INSIDE5) {
			result = new Point2D(x4, y2);
		}
		else if (anchor == ItemLabelAnchor.INSIDE6) {
			result = new Point2D(x3, y2);
		}
		else if (anchor == ItemLabelAnchor.INSIDE7) {
			result = new Point2D(x2, y2);
		}
		else if (anchor == ItemLabelAnchor.INSIDE8) {
			result = new Point2D(x2, y2);
		}
		else if (anchor == ItemLabelAnchor.INSIDE9) {
			result = new Point2D(x2, y3);
		}
		else if (anchor == ItemLabelAnchor.INSIDE10) {
			result = new Point2D(x2, y4);
		}
		else if (anchor == ItemLabelAnchor.INSIDE11) {
			result = new Point2D(x2, y4);
		}
		else if (anchor == ItemLabelAnchor.INSIDE12) {
			result = new Point2D(x3, y4);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE1) {
			result = new Point2D(x5, y6);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE2) {
			result = new Point2D(x6, y5);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE3) {
			result = new Point2D(x6, y3);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE4) {
			result = new Point2D(x6, y1);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE5) {
			result = new Point2D(x5, y0);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE6) {
			result = new Point2D(x3, y0);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE7) {
			result = new Point2D(x1, y0);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE8) {
			result = new Point2D(x0, y1);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE9) {
			result = new Point2D(x0, y3);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE10) {
			result = new Point2D(x0, y5);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE11) {
			result = new Point2D(x1, y6);
		}
		else if (anchor == ItemLabelAnchor.OUTSIDE12) {
			result = new Point2D(x3, y6);
		}
