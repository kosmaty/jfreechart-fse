...
public class StandardBarPainter implements BarPainter, Serializable {
	 ...
	 */
	@Override
	public void paintBar(GraphicsContext g2, BarRenderer renderer, int row,
			int column, RectangularShape bar, RectangleEdge base) {

		Paint itemPaint = renderer.getItemPaint(row, column);
		GradientPaintTransformer t = renderer.getGradientPaintTransformer();
		if (t != null && itemPaint instanceof LinearGradient) {
			itemPaint = t.transform((LinearGradient) itemPaint, bar);
		}
		g2.setFill(itemPaint);
		fillShape(g2, bar);

		// draw the outline...
		if (renderer.isDrawBarOutline()) {
			// && state.getBarWidth() > BAR_OUTLINE_WIDTH_THRESHOLD) {
			StrokeProperties stroke = renderer.getItemOutlineStroke(row, column);
	 ...
	 */
	@Override
	public void paintBarShadow(GraphicsContext g2, BarRenderer renderer, int row,
			int column, RectangularShape bar, RectangleEdge base,
			boolean pegShadow) {
		...
		Rectangle2D shadow = createShadow(bar, renderer.getShadowXOffset(),
				renderer.getShadowYOffset(), base, pegShadow);
		g2.setFill(renderer.getShadowPaint());
		fillRectangle(g2, shadow);
