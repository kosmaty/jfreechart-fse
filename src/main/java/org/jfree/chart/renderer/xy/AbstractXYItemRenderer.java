...
public abstract class AbstractXYItemRenderer extends AbstractRenderer
	 ...
	 */
	@Override
	public XYItemRendererState initialise(GraphicsContext g2, Rectangle2D dataArea,
	...
	public XYItemLabelGenerator getItemLabelGenerator(int series, int item) {
		XYItemLabelGenerator generator = this.itemLabelGeneratorList.get(series);
		if (generator == null) {
	...
	/**
	 * Returns the tool tip generator for a data item. If, for some reason, you
	 * want a different generator for individual items, you can override this
	 * method.
	 *
	 * @param series
	 *            the series index (zero based).
	 ...
	 *
	 * @param annotation
	 *            the annotation ({@code null} not permitted).
	 */
	@Override
	 ...
	 *            the annotation ({@code null} not permitted).
	 * @param layer
	 *            the layer ({@code null} not permitted).
	 */
	@Override
	 ...
	 *
	 * @param annotation
	 *            the annotation to remove ({@code null} not permitted).
	 *
	...
	/**
	 * Returns the range that should be set on the domain axis (x-axis) in order
	 * for this renderer to display all the items in the dataset.
	 *
	 * @param dataset
	...
	/**
	 * Returns the range of x-values in the specified dataset, either for all
	 * series or just the visible series (according to the setting of the
	 ...
	 *            include the interval (if any) for the dataset?
	 *
	 * @return The range ({@code null} if the dataset is {@code null} or empty).
	 *
	 * @since 1.0.13
	 ...
	 */
	@Override
	public void fillDomainGridBand(GraphicsContext g2, XYPlot plot, ValueAxis axis,
		...
		if (plot.getOrientation() == PlotOrientation.VERTICAL) {
			band = new Rectangle2D(Math.min(x1, x2), dataArea.getMinY(),
					Math.abs(x2 - x1), dataArea.getHeight());
		}
		else {
			band = new Rectangle2D(dataArea.getMinX(), Math.min(x1, x2),
		...
		if (paint != null) {
			g2.setFill(paint);
			g2.fillRect(band.getMinX(), band.getMinY(), band.getWidth(), band.getHeight());
		}
	 ...
	 */
	@Override
	public void fillRangeGridBand(GraphicsContext g2, XYPlot plot, ValueAxis axis,
			Rectangle2D dataArea, double start, double end) {
		...
		Rectangle2D band;
		if (plot.getOrientation() == PlotOrientation.VERTICAL) {
			band = new Rectangle2D(dataArea.getMinX(), Math.min(y1, y2),
					dataArea.getWidth(), Math.abs(y2 - y1));
		}
		else {
			band = new Rectangle2D(Math.min(y1, y2), dataArea.getMinY(),
	 ...
	 */
	@Override
	public void drawDomainGridline(GraphicsContext g2, XYPlot plot,
	...
	public void drawDomainLine(GraphicsContext g2, XYPlot plot, ValueAxis axis,
		...
		if (orientation.isHorizontal()) {
			line = newLine(dataArea.getMinX(), v, dataArea.getMaxX(),
					v);
	...
	/**
	 * Draws a line on the chart perpendicular to the x-axis to mark a value or
	 * range of values.
	 *
	 * @param g2
	 *            the graphics device.
	 ...
	 */
	@Override
	public void drawDomainMarker(GraphicsContext g2, XYPlot plot,
		...
		if (marker instanceof ValueMarker) {
			...
			if (label != null) {
				...
				Rectangle2D r = TextUtilities.calcAlignedStringBounds(label,
						...
						marker.getLabelTextAnchor());
				g2.setFill(marker.getLabelBackgroundColor());
				g2.fillRect(r.getMinX(), r.getMinY(), r.getWidth(), r.getHeight());
				g2.setFill(marker.getLabelPaint());
				TextUtilities.drawAlignedString(label, g2,
						(float) coords.getX(), (float) coords.getY(),
						marker.getLabelTextAnchor());
	...
	/**
	 * Draws a line on the chart perpendicular to the y-axis to mark a value or
	 * range of values.
	 *
	 * @param g2
	 *            the graphics device.
	 ...
	 */
	@Override
	public void drawRangeMarker(GraphicsContext g2, XYPlot plot, ValueAxis rangeAxis,
	 ...
	 * @return The coordinates for drawing the marker label.
	 */
	private Point2D calculateRangeMarkerTextAnchorPoint(GraphicsContext g2,
			PlotOrientation orientation, Rectangle2D dataArea,
			Rectangle2D markerArea, RectangleInsets markerOffset,
	 ...
	 */
	@Override
	public void drawAnnotations(GraphicsContext g2, Rectangle2D dataArea,
