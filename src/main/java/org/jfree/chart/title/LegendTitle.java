...
public class LegendTitle extends Title
	...
	public static final Font DEFAULT_ITEM_FONT = Font.font("SansSerif", FontWeight.NORMAL, FontPosture.REGULAR, 12);
	...
	public Object draw(GraphicsContext g2, Rectangle2D area, Object params) {
		Rectangle2D target = area;
		Rectangle2D hotspot = area;
		StandardEntityCollection sec = null;
		if (params instanceof EntityBlockParams
				&& ((EntityBlockParams) params).getGenerateEntities()) {
			sec = new StandardEntityCollection();
			sec.add(new TitleEntity(ShapeUtils.asShape(hotspot), this));
		}
		target = trimMargin(target);
		if (this.backgroundPaint != null) {
			g2.setFill(this.backgroundPaint);
			fillRectangle(g2, target);
		}
		BlockFrame border = getFrame();
		border.draw(g2, target);
		target = border.getInsets().trim2(target);
		BlockContainer container = this.wrapper;
		if (container == null) {
			container = this.items;
		}
