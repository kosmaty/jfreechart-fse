...
public class TextTitle extends Title implements Serializable, Cloneable,
	...
	/** The default font. */
	public static final Font DEFAULT_FONT = Font.font("SansSerif", FontWeight.BOLD, FontPosture.REGULAR,
			12);
	 ...
	 */
	@Override
	public Size2D arrange(GraphicsContext g2, RectangleConstraint constraint) {
	 ...
	 * @since 1.0.9
	 */
	protected Size2D arrangeNN(GraphicsContext g2) {
	 ...
	 * @since 1.0.9
	 */
	protected Size2D arrangeFN(GraphicsContext g2, double w) {
	 ...
	 * @since 1.0.9
	 */
	protected Size2D arrangeRN(GraphicsContext g2, Range widthRange) {
	 ...
	 * @return The content size.
	 */
	protected Size2D arrangeRR(GraphicsContext g2, Range widthRange,
			Range heightRange) {
		RectangleEdge position = getPosition();
		if (position == RectangleEdge.TOP || position == RectangleEdge.BOTTOM) {
	 ...
	 */
	@Override
	public Object draw(GraphicsContext g2, Rectangle2D area, Object params) {
		...
		if (params instanceof EntityBlockParams) {
			...
			if (p.getGenerateEntities()) {
				entity = new TitleEntity(ShapeUtils.asShape(area), this, this.toolTipText,
						this.urlText);
			}
		}
		area = trimBorder(area);
		if (this.backgroundPaint != null) {
			g2.setFill(this.backgroundPaint);
	 ...
	 */
	protected void drawHorizontal(GraphicsContext g2, Rectangle2D area) {
		...
		if (horizontalAlignment == HorizontalAlignment.LEFT) {
			...
			x = (float) getCenterX(titleArea);
			anchor = TextBlockAnchor.TOP_CENTER;
		}
		float y = 0.0f;
		RectangleEdge position = getPosition();
		if (position == RectangleEdge.TOP) {
			y = (float) titleArea.getMinY();
		}
	 ...
	 *            the area for the title.
	 */
	protected void drawVertical(GraphicsContext g2, Rectangle2D area) {
		Rectangle2D titleArea = area;
		g2.setFont(this.font);
		g2.setFill(this.paint);
		TextBlockAnchor anchor = null;
