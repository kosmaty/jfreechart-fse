...
public class CrosshairOverlay extends AbstractOverlay implements Overlay,
	 ...
	 *
	 * @param crosshair
	 *            the crosshair ({@code null} not permitted).
	 *
	 ...
	 *            the crosshair.
	 */
	protected void drawHorizontalCrosshair(GraphicsContext g2, Rectangle2D dataArea,
		...
		if (y >= dataArea.getMinY() && y <= dataArea.getMaxY()) {
			...
			if (crosshair.isLabelVisible()) {
				...
				Shape dataAreaShape = ShapeUtils.asShape(dataArea);
				if (!dataAreaShape.contains(hotspot.getBounds())) {
	 ...
	 *            the crosshair.
	 */
	protected void drawVerticalCrosshair(GraphicsContext g2, Rectangle2D dataArea,
		...
		if (x >= dataArea.getMinX() && x <= dataArea.getMaxX()) {
			...
			if (crosshair.isLabelVisible()) {
				...
				Shape hotspot = TextUtilities.calculateRotatedStringBounds(
						label, g2, xx, yy, alignPt, 0.0, TextAnchor.CENTER);
				if (!ShapeUtils.asShape(dataArea).contains(hotspot.getBounds())) {
