...
public class AxisSpace implements Cloneable, PublicCloneable, Serializable {
	...
	/**
	 * @deprecated Use {@link #shrink(Rectangle2D)}
	 */
	...
	public Rectangle2D reserved(Rectangle2D area, RectangleEdge edge) {
		...
		if (edge == RectangleEdge.TOP) {
			result = new Rectangle2D(
					area.getMinX(), area.getMinY(), area.getWidth(), this.top
					);
		}
		else if (edge == RectangleEdge.BOTTOM) {
			result = new Rectangle2D(
					area.getMinX(), area.getMaxY() - this.top,
					area.getWidth(), this.bottom
					);
		}
		else if (edge == RectangleEdge.LEFT) {
			result = new Rectangle2D(
					area.getMinX(), area.getMinY(), this.left, area.getHeight()
					);
		}
		else if (edge == RectangleEdge.RIGHT) {
			result = new Rectangle2D(
					area.getMaxX() - this.right, area.getMinY(),
