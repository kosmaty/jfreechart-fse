...
import static org.jfree.geometry.GeometryUtils.strokeRectangle;
...
public class MarkerAxisBand implements Serializable {
	 ...
	 */
	public double getHeight(GraphicsContext g2) {

		double result = 0.0;
		if (this.markers.size() > 0) {
			FontMetrics fontMetrics = TextUtilities.getFontMetrics(this.font);
			result = this.topOuterGap + this.topInnerGap + fontMetrics.getLineHeight()
	 ...
	 *            the text.
	 */
	private void drawStringInRect(GraphicsContext g2, Rectangle2D bounds, Font font,
			String text) {

		g2.setFont(font);
		Rectangle2D r = TextUtilities.getTextBounds(text, g2, font);
		double x = bounds.getMinX();
		if (r.getWidth() < bounds.getWidth()) {
			x = x + (bounds.getWidth() - r.getWidth()) / 2;
		}
		FontMetrics fontMetrics = TextUtilities.getFontMetrics(font);
		double y = bounds.getMaxY() - this.bottomInnerGap - fontMetrics.getDescent();
	 ...
	 *            the y-coordinate.
	 */
	public void draw(GraphicsContext g2, Rectangle2D plotArea, Rectangle2D dataArea,
			double x, double y) {
