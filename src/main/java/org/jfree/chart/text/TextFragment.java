...
public class TextFragment implements Serializable {
	...
	/** The default font. */
	public static final Font DEFAULT_FONT = Font.font("Serif", FontWeight.NORMAL, FontPosture.REGULAR, 12);
	...
	public Size2D calculateDimensions(GraphicsContext g2) {
		Rectangle2D bounds = TextUtilities.getTextBounds(this.text, g2, font);
	 ...
	 * @return the offset.
	 */
	public float calculateBaselineOffset(GraphicsContext g2, TextAnchor anchor) {
		float result = 0.0f;
		...
		if (anchor.isTop()) {
			...
			result = fm.getAscent() / 2.0f;
		}
		else if (anchor.isHalfHeight()) {
			result = fm.getAscent() / 2.0f - fm.getDescent() / 2.0f;
		}
